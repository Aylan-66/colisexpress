...
class _KycScreenState extends State<KycScreen> {
  ...
  static const _docTypes = [
    ('PieceIdentite', "Piece d'identite", Icons.badge),
    ...
    ('Selfie', 'Selfie avec piece d\'identite', Icons.face),
  ];
  ...
  Future<void> _upload(String typeDocument) async {
    ...
    setState(() {
      _uploadingType = typeDocument;
      _success = null;
      _error = null;
    });
    final res = await api.uploadKycDocument(typeDocument, File(file.path));
    ...
    if (res.containsKey('error')) {
      ...
      setState(() {
        _uploadingType = null;
        _success = 'Document soumis !';
      });
      final status = await api.getKycStatus();
      if (mounted) {
        setState(() {
          _status = status.containsKey('error') ? _status : status;
        });
        ...
        child: ListTile(
          ...
          trailing: Row(
            ...
            children: [
              ...
              IconButton(
                icon: Icon(
                    existing == null ? Icons.add_a_photo : Icons.refresh,
                    size: 20),
                color: AppTheme.primary,
                onPressed: isUploading || isOtherUploading
                    ? null
                    : () => _upload(type),
              ),
            ],
  ...
  String get _kycStatut =>
      _status?['statutKyc']?.toString() ?? 'NonSoumis';
  ...
  String get _kycLabel => switch (_kycStatut) {
        'Valide' => 'Verifie - votre point relais est actif',
        'EnAttente' => 'En attente de validation par l\'admin',
        'Rejete' => 'Rejete - veuillez re-soumettre vos documents',
